...
def read_glove_embeddings(corpus):
    gloveWords = pd.read_table("embeddings/glove/glove.840B.300d.txt", sep=" ", index_col=0, header=None, quoting=csv.QUOTE_NONE)

    index2word = get_uniq_elems(corpus)
    index2word = [PAD_TOKEN, UNK_TOKEN] + index2word
    word2index = ddict(lambda: index2word.index(UNK_TOKEN), {w: i for i, w in enumerate(index2word)})

    embeddings = np.zeros((len(index2word), 300))   

    length = len(index2word)
    for i, word in enumerate(index2word):
        if i%10000 == 0:
            print("{}/{}".format(i,length))
        try:
            embeddings[i] = gloveWords.loc[word].as_matrix()
        except KeyError:
            pass
